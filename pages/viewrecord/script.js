...
function editActivityInit(record_id, component_id, activity) {
    ENABLE_CONSOLELOG &&
...
function populateComponentList(record) {
    ...
    `;
        ...
        Table.classList.add("w-full", "border", `border-${record.color}-500`, "rounded-b", "bg-white", "whitespace-nowrap");
     ...
     `;
        ...
        ${activity.components.reduce((sum, component) => sum + component.score, 0)}</span>
      ...
      `;
        ...
        NewActivityButton.classList.add("border-2", "border-green-400", "text-green-400", "hover:text-white", "hover:bg-green-400", "rounded", "py-2", "px-3");
        ...
        let totalScore = 0;
        for (let activity of component.activities) {
            totalScore += activity.components.reduce((sum, component) => sum + component.score, 0);
        }
      ...
      `;
          ...
          `);
            ...
            let getActivityTotalScore = component.activities.reduce((sum, activity) => sum +
                activity.components.reduce((sum, component) => sum + component.score, 0), 0);
        ...
        <td class='px-6 py-3 text-center border-x border-x-neutral-400 font-semibold'>
        ${LinearScale(0, 100, 0, component.score, (studentTotalScore / getActivityTotalScore) * 100 || 0) || 0} %
        </td>
      `);
