...
function editSectionInit(data: Section) {
	...
	SectionColorRadio.forEach((radioButton: HTMLInputElement) => {
...
function addStudentInit(data: any) {
...
function editStudentInit(section: string, data: any) {
	document.getElementById("StudentModalCloseButton")!.onclick = () => {
		StudentModal.hide();
	};
	document.getElementById("StudentModalCancelButton")!.onclick = () => {
...
function deleteStudentInit(section: string, data: any) {
	document.getElementById("DeleteModalCloseButton")!.onclick = () => {
		DeleteModal.hide();
	};
	document.getElementById("DeleteModalCancelButton")!.onclick = () => {
...
function sendDataSection(title: string, data: any) {
...
function sendDataStudent(title: string, data: any) {
	Action_Spinner.classList.remove("hidden");
...
function createSection(row: any) {
  ...
  S.Y. ${dateFormatter.format(syStart)} - ${dateFormatter.format(syEnd)}`;
	...
	SectionAddButton.classList.add(
		"rounded",
		"py-1",
		"px-2",
		"font-medium",
	...
	SectionEditButton.classList.add(
		"rounded",
		"py-1",
		"px-2",
		"font-medium",
	...
	SectionDeleteButton.classList.add(
		"rounded",
		"py-1",
		"px-2",
		"font-medium",
	...
	SectionHideButton.addEventListener("click", () => {
		document.getElementById(row.id + "-TABLE")?.classList.toggle("hidden");
  ...
  `;
	...
	for (let student of row.students) {
		...
		if (student.gender === "Male") genderColor = "text-blue-500";
		if (student.gender === "Female") genderColor = "text-pink-500";
  ...
  `;
		...
		StudentEditButton.classList.add(
			"rounded",
			"py-2",
			"px-3",
			"font-medium",
		...
		StudentDeleteButton.classList.add(
			"rounded",
			"py-2",
			"px-3",
			"font-medium",
	...
	// append to SECTION ACTIONS CONTAINER
...
export function handleResponseData(data: any) {
	...
	if (data.status === 403) window.location.href = Routes.LOGIN_PAGE;
	if (data.data.user) window.location.href = Routes.HOME_PAGE;
	if (data.data.redirect) window.location.href = Routes.DASHBOARD_PAGE;
}
